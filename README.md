...
```shell
wget https://github.com/textmagic/textmagic-rest-cpp-v2/archive/v2.0.809.tar.gz && \
tar zxf v2.0.809.tar.gz && \
rm -f v2.0.809.tar.gz && \
cd textmagic-rest-cpp-v2-2.0.809
```
Build using cmake
```shell
cmake . && cmake --build .
```
The output library file will be placed in the `textmagic-rest-cpp-v2-2.0.809/lib` directory:

## Usage Example
In the example below, we assume that you moved the library sources `textmagic-rest-cpp-v2-2.0.809` directory to your test project root directory.
...
set_property(TARGET textmagic_client PROPERTY IMPORTED_LOCATION "${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.809/lib/libtextmagic_client.so")

target_include_directories (app PRIVATE ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.809 ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.809/model ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.809/api)
...
#include <iostream>
#include <fstream>
#include "textmagic-rest-cpp-v2-2.0.809/ApiClient.h"
#include "textmagic-rest-cpp-v2-2.0.809/ApiConfiguration.h"
#include "textmagic-rest-cpp-v2-2.0.809/api/TextMagicApi.h"
