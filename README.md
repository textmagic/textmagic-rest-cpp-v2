...
```shell
wget https://github.com/textmagic/textmagic-rest-cpp-v2/archive/v2.0.819.tar.gz && \
tar zxf v2.0.819.tar.gz && \
rm -f v2.0.819.tar.gz && \
cd textmagic-rest-cpp-v2-2.0.819
```
Build using cmake
```shell
cmake . && cmake --build .
```
The output library file will be placed in the `textmagic-rest-cpp-v2-2.0.819/lib` directory:

## Usage Example
In the example below, we assume that you moved the library sources `textmagic-rest-cpp-v2-2.0.819` directory to your test project root directory.
...
set_property(TARGET textmagic_client PROPERTY IMPORTED_LOCATION "${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.819/lib/libtextmagic_client.so")

target_include_directories (app PRIVATE ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.819 ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.819/model ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.819/api)
...
#include <iostream>
#include <fstream>
#include "textmagic-rest-cpp-v2-2.0.819/ApiClient.h"
#include "textmagic-rest-cpp-v2-2.0.819/ApiConfiguration.h"
#include "textmagic-rest-cpp-v2-2.0.819/api/TextMagicApi.h"
