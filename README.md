...
```shell
wget https://github.com/textmagic/textmagic-rest-cpp-v2/archive/v2.0.812.tar.gz && \
tar zxf v2.0.812.tar.gz && \
rm -f v2.0.812.tar.gz && \
cd textmagic-rest-cpp-v2-2.0.812
```
Build using cmake
```shell
cmake . && cmake --build .
```
The output library file will be placed in the `textmagic-rest-cpp-v2-2.0.812/lib` directory:

## Usage Example
In the example below, we assume that you moved the library sources `textmagic-rest-cpp-v2-2.0.812` directory to your test project root directory.
...
set_property(TARGET textmagic_client PROPERTY IMPORTED_LOCATION "${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.812/lib/libtextmagic_client.so")

target_include_directories (app PRIVATE ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.812 ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.812/model ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-v2-2.0.812/api)
...
#include <iostream>
#include <fstream>
#include "textmagic-rest-cpp-v2-2.0.812/ApiClient.h"
#include "textmagic-rest-cpp-v2-2.0.812/ApiConfiguration.h"
#include "textmagic-rest-cpp-v2-2.0.812/api/TextMagicApi.h"
