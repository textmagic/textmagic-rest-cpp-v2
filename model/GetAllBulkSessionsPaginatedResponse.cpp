...
void GetAllBulkSessionsPaginatedResponse::fromJson(web::json::value& val)
{
    ...
    }
    {
        ...
        for( auto& item : val[utility::conversions::to_string_t("resources")].as_array() )
        {
            if(item.is_null())
            {
                m_Resources.push_back( std::shared_ptr<BulkSession>(nullptr) );
            }
            else
            {
                std::shared_ptr<BulkSession> newItem(new BulkSession());
...
void GetAllBulkSessionsPaginatedResponse::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
...
std::vector<std::shared_ptr<BulkSession>>& GetAllBulkSessionsPaginatedResponse::getResources()
{
    return m_Resources;
}

void GetAllBulkSessionsPaginatedResponse::setResources(std::vector<std::shared_ptr<BulkSession>> value)
{
    m_Resources = value;
