...
bool ListImage::fromJson(const web::json::value& val)
{
    bool ok = true;
...
bool ListImage::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
...
void ListImage::setHref(const utility::string_t& value)
{
    m_Href = value;
    m_HrefIsSet = true;
}

bool ListImage::hrefIsSet() const
{
    return m_HrefIsSet;
}

void ListImage::unsetHref()
{
    m_HrefIsSet = false;
}
